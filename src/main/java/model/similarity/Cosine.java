...
public class Cosine extends Similarity {
    ...
    public Cosine(Connection connection) throws SQLException {
        ...
        calculateCosine = connection.prepareStatement("SELECT i.idDoc, SUM((q.weight*i.weight)/SQRT((SELECT SUM(q1.weight) FROM QUERY q1)*(SELECT SUM(i2.weight) FROM SPATIA.INVERTEDINDEX i2 WHERE i2.idDoc=i.idDoc))) as sim\n" +
