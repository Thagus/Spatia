...
public class SpanishPorterStemmer {
    ...
    private static boolean endsin(String word, String suffix) {
        return strlen(word) >= strlen(suffix) && (substr(word, -strlen(suffix)).equals(suffix));
    }
    ...
    private static String substr(String word, int start, int length) {
        if (start == length) {
            return "";
        } else {
            if ((start >= 0)) {
    ...
    private static String substr(String word, int i) {
